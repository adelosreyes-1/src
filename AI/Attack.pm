...
sub process {
	...
	if (
		...
		# SIMPLEST FIX - Works immediately, no function calls needed
# Replace lines 70-88 in AI/Attack.pm

		my $target = Actor::get($ID);
		if ($target && $config{attackChangeTarget}) {
			...
			if (@aggressives) {
				# Find the CLOSEST aggressive monster
				my $myPos = $char->{pos_to};
				my $closestDist = 9999;
				my $closestTarget;
				
				foreach my $monID (@aggressives) {
					my $monster = Actor::get($monID);
					next unless $monster;
					
					my $monPos = $monster->{pos_to};
					my $dist = blockDistance($myPos, $monPos);
					
					# Skip if LOS check fails (if enabled)
					if ($config{attackCheckLOS}) {
						next unless $field->checkLOS($myPos, $monPos, $config{attackCanSnipe});
					}
				...
				# Decide if we should switch to the closest target
				if ($closestTarget && $closestTarget ne $ID) {
					my $currentDist = blockDistance($myPos, $target->{pos_to});
					my $new_target = Actor::get($closestTarget);
					my $newDist = blockDistance($myPos, $new_target->{pos_to});
					my $shouldSwitch = 0;
					
					# Always switch from non-aggressive to aggressive
					if (!$target_is_aggressive) {
						$shouldSwitch = 1;
						warning TF("Target is not aggressive: %s, switching to aggressive: %s.\n", $target, $new_target), 'ai_attack';
					}
					# Switch between aggressives if new one is significantly closer
					elsif ($target_is_aggressive) {
						my $minDiff = $config{attackChangeTarget_minDistance} || 3;
						if ($newDist + $minDiff < $currentDist) {
							$shouldSwitch = 1;
							warning TF("Switching to closer target: %s (dist:%d) from %s (dist:%d).\n", 
								$new_target, $newDist, $target, $currentDist), 'ai_attack';
						}
					}
